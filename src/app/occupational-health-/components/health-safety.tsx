...
import { healthAndSafety } from "@/constants";
...
const HealthSafety = ({ slug }: PageProps) => {
  const [getData, setData] = useState<
    (typeof healthAndSafety)[0] | undefined
  >();

  const router = useRouter();

  useEffect(() => {
    const filterData = healthAndSafety.find(
      (solution) => solution.slug === `/occupational-health-/${slug}`,
    );
    setData(filterData);
  }, [slug]);
          ...
          >
            <div className="grid lg:grid-cols-2 gap-20 items-center">
              ...
              >
                ...
                >
                  <Image
                    src={getData.img || "/placeholder.svg"}
                    alt={`${getData.title} Overview`}
                    width={800}
                    height={600}
